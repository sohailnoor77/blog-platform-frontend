...
const fetchBlogs = async (append = false) => {
    try {
        ...
        // api call
        const { data } = await api.get(`/author/blogs`, {
            params: {
...
<template>
    ...
    <TransitionGroup name="fade" tag="div" :class="[
        'mx-auto mt-6 grid max-w-2xl grid-cols-1 gap-8 border-t border-gray-200 pt-6 sm:mt-10 sm:pt-10 lg:mx-0 lg:max-w-none lg:grid-cols-3',
        state.blogs.length === 0 ? 'h-screen' : ''
    ]">
        <article v-for="blog in state.blogs" :key="blog.id" class="flex max-w-xl flex-col items-start">
            ...
            <div class="group relative">
                <h3 class="mt-3 text-lg/6 font-semibold text-gray-900 group-hover:text-gray-600">
                    <RouterLink :to="'/author/blogs/' + blog.id">
